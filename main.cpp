...
UBENCH_EX(iv, naive_bsv) {
  ...
  for (auto i = 0; i < SIZE_N; ++i) {
    ...
    data.rate[i] = 0.05;
    data.vol[i] =
        0.2 + 0.4 * static_cast<float>(rand()) / static_cast<float>(RAND_MAX);
    data.px[i] = bsPrice(true, data.ul[i], data.tte[i], data.strike[i],
                         data.rate[i], data.vol[i]);
  }

  UBENCH_DO_BENCHMARK() {
    for (auto i = 0; i < SIZE_N; ++i) {
      data.iv[i] = bisectIV(true, data.ul[i], data.tte[i], data.strike[i],
                            data.rate[i], data.px[i]);
    }
  }
...
UBENCH_EX(iv, naive_bs) {
  ...
  for (auto i = 0; i < SIZE_N; ++i) {
    ...
    data[i].vol =
        0.2 + 0.4 * static_cast<float>(rand()) / static_cast<float>(RAND_MAX);
    data[i].px = bsPrice(true, data[i].ul, data[i].tte, data[i].strike,
                         data[i].rate, data[i].vol);
  }

  UBENCH_DO_BENCHMARK() {
    for (auto i = 0; i < SIZE_N; ++i) {
      data[i].iv = bisectIV(true, data[i].ul, data[i].tte, data[i].strike,
                            data[i].rate, data[i].px);
    }
  }
...
UBENCH_EX(iv, avx_bsv) {
  ...
  UBENCH_DO_BENCHMARK() {
    ...
    for (auto i = 0; i < SIZE_N; i += 16)
      bisectIVVec(true, u.load(data.ul + i), t.load(data.tte + i),
                  s.load(data.strike + i), r.load(data.rate + i),
                  p.load(data.px + i))
          .store(data.iv + i);
  }
...
UBENCH_EX(iv, avx_bsv512) {
  ...
  for (auto i = 0; i < SIZE_N / 16; ++i) {
    ...
    data.vol[i] =
        0.2 + 0.4 * static_cast<float>(rand()) / static_cast<float>(RAND_MAX);
    data.px[i] = bsPriceVec(true, data.ul[i], data.tte[i], data.strike[i],
                            data.rate[i], data.vol[i]);
  }
  ...
  UBENCH_DO_BENCHMARK() {
    ...
    for (auto i = 0; i < SIZE_N / 16; i++)
      data.iv[i] = bisectIVVec(true, data.ul[i], data.tte[i], data.strike[i],
                               data.rate[i], data.px[i]);
  }
...
UBENCH_EX(iv, avx_bs) {
  ...
  UBENCH_DO_BENCHMARK() {
    for (auto i = 0; i < SIZE_N; i += 16) {
      ...
      scatter<BS_IV>(bisectIVVec(true, ul, tte, str, rate, px),
                     (float *)(data + i));
    }
  }
...
UBENCH_EX(pricer, avx_bsv) {
  ...
  UBENCH_DO_BENCHMARK() {
    ...
    for (auto i = 0; i < SIZE_N; i += 16) {
      ...
      bsPriceVec(true, u.load(data.ul + i), t, u.load(data.strike + i),
                 r.load(data.rate + i), v)
          .store(data.px + i);
    }
  }
}
...
#pragma omp parallel
    {
      ...
      Vec16f u, t, s, r, v;
      for (auto i = N * ii; i < (ii + 1) * N; i += 16)
        bsPriceVec(true, u.load(data.ul + i), t.load(data.tte + i),
                   s.load(data.strike + i), r.load(data.rate + i),
                   v.load(data.vol + i))
            .store(data.px + i);
    }
...
UBENCH_EX(pricer, avx_bs) {
  ...
  UBENCH_DO_BENCHMARK() {
    for (auto i = 0; i < SIZE_N; i += 16) {
      ...
      scatter<BS_THEO>(bsPriceVec(true, ul, tte, str, rate, vol),
                       (float *)(data + i));
    }
  }
