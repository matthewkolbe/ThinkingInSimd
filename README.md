...
The ubiquity and effectiveness in AVX intrinsics in the most popular programming languages has opened up a whole new paradigm for software optimization. Behind the scenes, compilers have embradced the AVX instruction sets, and will automatically apply them when appropriate ("autovectorization"). But autovectorization is generally only applied on shallow/obvious use cases, whereas manually applying intrinsic functions can give you power far beyond what compilers can accomplish today. Don't just take it from me, here's Linus Torvalds
